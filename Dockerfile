...
CMD set -xe; python manage.py migrate --noinput; python manage.py shell -c "import os; from django.contrib.auth import get_user_model; User=get_user_model(); username=os.environ.get('DJANGO_SUPERUSER_USERNAME'); password=os.environ.get('DJANGO_SUPERUSER_PASSWORD'); u=User.objects.filter(username=username).first(); u.set_password(password); u.is_superuser=True; u.is_staff=True; u.save() if u else User.objects.create_superuser(username=username, email=os.environ.get('DJANGO_SUPERUSER_EMAIL',''), password=password); print('Production admin password configured')" ; gunicorn ecommerce.wsgi:application